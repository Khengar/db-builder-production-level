...
export default function TableNode({ table }: Props) {
  ...
  return (
    ...
    >
      {/* --- HEADER --- */}
      <div
        className={`h-10 border-b border-white/5 flex items-center justify-between px-3 cursor-grab active:cursor-grabbing rounded-t-lg transition-colors ${isSelected ? "bg-violet-500/10" : "bg-zinc-900/50"
          }`}
...
const Handle = ({ side, onMouseDown, onMouseUp, isConnected }: any) => (
  // Container: Invisible 20px hit area
  <div
    className={`
      absolute top-1/2 -translate-y-1/2 w-5 h-5 z-20 cursor-crosshair flex items-center justify-center
      ${side === 'left' ? '-left-[10px]' : '-right-[10px]'}
    `}
    onMouseDown={(e) => { e.stopPropagation(); onMouseDown(); }}
    onMouseUp={(e) => { e.stopPropagation(); onMouseUp(); }}
  >
    {/* Visual Dot: 10px (Standard Size) */}
    <div
      className={`
        w-2.5 h-2.5 rounded-full border border-zinc-900 transition-all
        ${isConnected
